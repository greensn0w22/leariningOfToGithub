...
#define E_STEPSIZE	54000	//236250	//0.0236250
...
void printer3d::execute(void){
	...
	if(gCodeList.G0 == true){
		...
		static bool _XComplete = 0, _YComplete = 0, _ZComplete = 0, _speed = 1;
...
bool printer3d::pixelDraw(bool g0g1, bool X, bool Y, bool Z, bool E){
	...
	uint32_t deltaX, deltaY, deltaOther = 0;
	uint32_t stepRatioX, stepRatioY, stepRatioOther;
	uint32_t stepPerXYMov;
	uint32_t otherCounter = 0;
	uint8_t	 _ZOrE = 0;
	uint16_t i;

	if((Z == true) || (E ==  true)){
		deltaX = abs(Ginf.ObjectiveX - Ginf.X);
		deltaY = abs(Ginf.ObjectiveY - Ginf.Y);
		stepRatioX = deltaX/XY_STEPSIZE;
		stepRatioY = deltaY/XY_STEPSIZE;

		if(Z == true){
			deltaOther = abs(Ginf.ObjectiveZ - Ginf.Z);
			stepRatioOther = deltaOther/Z_STEPSIZE;
			_ZOrE = 2;
		}
		else if(E == true){
			deltaOther = abs(Ginf.ObjectiveE - Ginf.E);
			stepRatioOther = deltaOther/E_STEPSIZE;
			_ZOrE = 3;
		}
		stepPerXYMov = (stepRatioX+stepRatioY)/stepRatioOther;
	}
	...
	*/
	...
	if((dx!= 0) || (dy!= 0)){
		for (; x0<x1; ) //THIS BLOODY LOOP
		{
			if(deltaOther){ //IF THERE'S A Z OR E
				if(otherCounter%stepPerXYMov == 0){
					fillMoveStruct(&move.mouvement[_step4], modulo, _ZOrE);
					if(modulo++==3)
						_step4++;
					modulo %= 4;
					move.numberOfStep++;
			...
			x0+= XY_STEPSIZE;
			//X MOUVEMENT UNLESS steep = 1 THEN Y MOUVEMENT
			if (steep)
			...
			if (err < 0) //Y MOUVEMENT UNLESS steep = 1 THEN X MOUVEMENT
			{
				y0 += ystep;
				err += dx;
				if (steep)
